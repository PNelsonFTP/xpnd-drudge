...
(() => {
  ...
  function renderBanners(data) {
    ...
    if (data.generatedAt && ageHours(data.generatedAt) > STALE_DATA_HOURS) {
      parts.push(
        `<div class="banner">Headlines may be delayed — last refresh was over ${STALE_DATA_HOURS} hours ago.
          <button type="button" data-action="refresh">Refresh now</button></div>`
      );
    }
  ...
  function render() {
    ...
    el.footerRight.textContent = `built ${new Date(data.generatedAt).toLocaleString()} · news cached 15m · quotes 10m`;
  ...
  $("btn-csv").addEventListener("click", () => {
    window.open("/api/alerts.csv", "_blank");
  });
  ...
  async function load(refresh = false, silent = false) {
    ...
    if (!silent) {
      ...
      el.status.textContent = refresh ? "Refreshing headlines…" : "Loading headlines…";
      if (!state.data) el.skeleton.hidden = false;
    }
    try {
      const res = await fetch(`/api/dashboard?per_company=10${refresh ? "&refresh=true" : ""}`);
      ...
      render();
      if (refresh && !silent) toast("Headlines refreshed");
      try {
